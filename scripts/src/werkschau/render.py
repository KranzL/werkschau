...
def _about_text() -> str:
    return (
        ...
        "as IC leverage grows (roughly 600–700 minutes per week for DE1–DE3, 600 for Senior, "
        "400 for Staff, 300 for Senior Staff, 250 for Principal, 180 for Senior Principal, "
        "120 for Distinguished) and scaled per role: SWE 1.0, AE 0.9, MLE 0.8, DS 0.55, DA 0.5. "
        "Effort is weighted by per-commit complexity. Each commit is categorized by message "
        "prefix and file paths into Features, Bug fixes, Refactor, Infrastructure, Tests, or "
        "Documentation; the dominant category appears when one exceeds half of the week's effort. "
        "Manager rollups are the median of each manager's direct reports — the manager's own score "
        "appears separately in the ledger."
    )
